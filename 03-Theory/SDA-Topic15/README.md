...
- **Темата започва малко по-надолу**
- възможно е да има ненужни функции
- Всяка функции от имплементацията трябва да я опишете какво и как го прави!!!
- Всяка сложност трябва да бъде обоснована защо се получава!!!
- Темата така разгърната носи 6.00 (тествано)

## Зададени въпроси към преподавател
1. На пета точка за имплементация, какво точно трябва да накодя за кореново дърво? Не мога да разбера какъв точно трябва да бъде интерфейса на структурата?
2. Нужно ли е да пиша Move semantics в имплементациите?
3. Грешките по член функциите в имплементациите трябва ли да са по-специфични от std::exception?
4. С какви exception safety guarantees трябва да ми бъдат имплементациите?
5. На точката за LinkedList трябва ли да се коментира имплементация, когато има поинтър към опашка и когато няма? Или се приема, че винаги говорим за имплементация с поинтър към опашката?
6. Трябва ли на BST да се обясни как работят insert, find, remove?
7. Има ли значение дали ще напиша статична, динамична или свързана имплементация на структурите (например опашката)? Понеже статичната е най-бърза за разписване, но това ще се отрази ли на точките ми?

Отговори:
1. За кореново дърво обикновено обяснявате какво точно дърво реализирате (например директорийна структура), показвате операции по добавяне (по някакви критерии), съответно търсене (ако има смисъл) и обхождания (ширина, дълбочина).
2. Не
3. Не е нужно да имате изключения - може да връщате bool като резултат или друг избран от вас начин за сигнализиране на грешките
4. Достатъчно е да гарантирате коректно състояние и да няма течове на ресурси (weak guarantee).
5. Няма значение, стига да сте описали какво и защо правите (и да сте съобразили функциите, които предлагате с избора). Например не е ОК да нямате указател към края, но да посочите като удобна функционалност добавяне в двата края.
6. За всяка структура освен кода трябва да имате и обяснения към него. В частност и за BST
7. Няма значение, стига да сте описали добре нещата.
...
### Характеристики на статична, динамична и свързана реализация
- Статична реализация - използва масив с фиксиран размер, който не може да се променя по време на изпълнение. Ограничение е, че ако опашката е пълна, не може да се добавят нови елементи.
- Динамична реализация - използва динамично заделяне на памет, което позволява опашката да расте и се свива по време на изпълнение. Тоест ако текущо заделената памет се напълни елементи, опашката се разширява(като се заделя нова по-голяма памет), след което елементите се копират в новата памет, а старата се освобождава.
- Свързана реализация - използва свързани елементи (може да се използва вътрешно linked list), където всеки елемент има указател към следващия. Това позволява динамично добавяне и премахване на елементи, без да се налага да се заделя голяма памет предварително.
...
- Балансиране - Дърво, за което височината на дървото се поддържа автоматично възможно най-малка спрямо броя на елементите в него с цел оптимални операции откъм търсене, премахване и добавяне. В случай на AVL балансираните дървета за всеки възел имаме свойството, че височината на лявото му поддърво се различава от височината на дясното поддърво с максимум единица.
...
- При принтиране в реда ляво-корен-дясно ще получим всички елементи в нарастващ ред (съответно при дясно-корен-ляво в намаляващ)
